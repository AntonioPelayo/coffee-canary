...
def make_roaster_distribution(df_bags: pd.DataFrame, df_roasters: pd.DataFrame):
    if 'roaster_id' not in df_bags.columns or df_bags.empty or df_roasters.empty:
        return px.bar(title='No data available')
    # Merge to get roaster names
    merged = df_bags.merge(df_roasters[['id', 'name']], left_on='roaster_id', right_on='id', how='left', suffixes=("_bean", "_roaster"))
...
def make_cumulative_weight_line(beans_df: pd.DataFrame, roasters_df: pd.DataFrame):
    ...
    for extra in ['name', 'Blend Name', 'roaster_id']:
    ...
    merged['Roaster'] = merged['name_roaster']
    merged['Blend Name'] = merged['name_bean']
    merged['Bag Weight (g)'] = merged['weight_g']
    fig = px.line(
        merged,
        x=date_col,
        ...
        markers=True,
        hover_data={
            'Blend Name': True if 'Blend Name' in merged.columns else False,
            'Roaster': True if 'Roaster' in merged.columns else False,
            date_col: False,
            'Bag Weight (g)': True if 'Bag Weight (g)' in merged.columns else False,
...
def make_roaster_location_map(beans_df: pd.DataFrame, roasters_df: pd.DataFrame):
    if beans_df.empty or roasters_df.empty:
        return go.Figure()
    used = set(beans_df['name'].dropna().unique())
